...
A clean, native **macOS** menu-bar player for [WQXR](https://www.wqxr.org/) — New York's
classical music station. Play and pause are always one click away, up by your clock, showing
the current show and host, the composer and piece, and what just played — and never a stray
browser window.

**▶ [Download for Mac](https://content.stevenowicki.com/wqxr/)** — free, Apple-signed &
notarized. Or grab a `.dmg` from [Releases](https://github.com/stevenowicki/wqxr-streamer/releases/latest).
...
- **The menu-bar icon gently breathes while audio is playing** — a quiet cue for where
...
For a distributable, signed + notarized DMG (no Gatekeeper warning for downloaders), see
**[DISTRIBUTION.md](DISTRIBUTION.md)** — it covers the Developer ID certificate, notarization
credentials (kept in a git-ignored `.env`), and the build commands. The code is built with
Electron and is largely cross-platform, but only the macOS build is produced today.

`npm run deploy:site` packages `public/` and ships it to the download site.
